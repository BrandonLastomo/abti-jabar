...
<div class="p-6 lg:p-10 min-h-screen bg-gray-50/50">
    ...
    <div class="mb-10 flex items-center justify-between">
        <div>
            <h1 class="font-heading font-extrabold text-4xl text-gray-900 tracking-tight mb-2">Transfer & Mutations</h1>
            <p class="text-gray-500 font-medium">Verify club transfer requests submitted by players.</p>
        </div>
    ...
    <div class="flex flex-wrap items-center gap-3 mb-6">
        @foreach($filters as $key => $filterData)
            ...
            @endphp
            <a href="{{ route('admin.mutations.index', ['filter' => $key]) }}"
    ...
    <div class="bg-white/90 backdrop-blur-2xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:rounded-3xl overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100/50">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @if($mut->status === 'pending')
                                    ...
                                    <button type="button" onclick="openRejectModal({{ $mut->id }})"
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            ...
                            <p class="text-gray-500 font-medium">No mutations found for this status.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/30">
            {{ $mutations->appends(['filter' => $filter])->links() }}
        </div>
        @endif
...
<div id="rejectModal" class="fixed inset-0 z-[100] flex items-center justify-center hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="relative bg-white rounded-3xl overflow-hidden shadow-2xl transform transition-all sm:max-w-lg w-full p-8 m-4">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h3 class="text-xl font-heading font-extrabold text-gray-900" id="modal-title">Tolak Pengajuan Mutasi</h3>
