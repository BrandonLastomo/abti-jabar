                ...
                <div class="space-y-6">
                    ...
                    @foreach($educationLevels as $level)
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-4 rounded-lg border border-gray-100 shadow-sm">
                            <div>
                                <label for="{{ $nameField }}" class="block text-sm font-bold text-gray-700 mb-1">Nama Institusi {{ $level['label'] }}</label>
                                <input type="text" name="{{ $nameField }}" id="{{ $nameField }}" value="{{ old($nameField, $education->$nameField) }}" class="w-full rounded-xl border-gray-200 bg-gray-50/50 shadow-sm focus:border-red-500 focus:ring-red-500 focus:bg-white transition-colors text-sm">
